...
```{r}
data.wide.GN <- data.long %>% reshape2::dcast(subject+hotspot~mode,  value.var="time")
#print(data.wide.GN)
```

```{r}
wilcoxon.res <- wilcox.test(data.wide.GN$N, data.wide.GN$G, alternative="two.sided", paired=TRUE, conf.int=TRUE)

summary.wilcoxon <- wilcoxon.res %>% tidy
...
DT::datatable(
  ...
  caption = htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: center;',
    glue('Fig {tc<-tc+1}: Signed-Rank Wilcoxon Test summary')
  )
) %>% formatRound(1:5, 10)
```
...
```{r}
glmm1 <- lmer(time ~mode + (1|id/mode), data=data.long.time2)
glmm1
```
