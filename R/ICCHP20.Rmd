...
```{r}
lme0 <- lmer(data = data.long, formula = Time ~ Condition + (1|Participant), REML=FALSE)
lme0 #%>% broom::tidy()
```

```{r}
lme1 <- lmer(data = data.long, formula = Time ~ (1|Participant), REML=FALSE)
lme1
```

```{r}
lme2 <- lmer(data = data.long, formula = Time ~ Condition + (Condition|Participant), REML=FALSE)
lme2
```
