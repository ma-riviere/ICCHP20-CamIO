...
data.long <- data.wide %>%
  ...
  #mutate(Hotspot = factor(Hotspot, levels = hs.order, ordered = TRUE)) %>%
...
```{r}
wsrt1 <- wilcox.test(formula=Time ~ Condition, data=data.long.wil, paired=TRUE, alternative="two.sided")
wsrt1 %>% tidy
```
...
```{r}
wrst.z / sqrt(nrow(NG))
data.long.wil %>% rstatix::wilcox_effsize(formula=Time ~ Condition, paired=TRUE)
```
...
hist1
ggsave("../res/img/hist1.png", hist1, device = "png", dpi=320)
hist2
ggsave("../res/img/hist2.png", hist2, device = "png", dpi=320)
```
...
```{r warnings=T}
test <- data.long %>% 
  select(Hotspot, Participant, Condition, Time) %>%
  group_by(Condition) %>% 
  mutate(Hotspot = factor(Hotspot, levels=hs.order)) %>% 
  ungroup %>%
  arrange(Participant, Hotspot) %T>% print

#data.long$Hotspot <- factor(data.long$Hotspot, levels = hs.order)
